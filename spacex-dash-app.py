...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig=px.pie(specific_df,
                   names='class',
                   title='Total Success Launches for a Specific Site')
        return fig
# TASK 4:
...
def scatter(entered_site, payload_slider):
    ...
    if entered_site == 'ALL':
        fig=px.scatter(data_frame=filtered_data,
            ...
            title= 'Payload Mass (kg) and Launch Site Success for All Sites')
        return fig
    else:
        specific_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        data_frame = specific_df[(specific_df['Payload Mass (kg)']>=payload_slider[0])
        &(specific_df['Payload Mass (kg)']<=payload_slider[1])]
        fig=px.scatter(data_frame,
                       x='Payload Mass (kg)',
                       y='class',
                       color='Booster Version Category',
                       title='Success Count on Payload Mass for a Specific site')
        return fig
